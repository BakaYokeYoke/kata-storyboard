...
const I18N = [
  ...
  ['Cliquez le cercle ● pour valider la lecture', 'Click the circle ● to confirm the reading'], ['Lu ✓', 'Read ✓'],
  ['Dérouler le standard', 'Run the standard'], ['≈ 90 % du time block', '≈ 90 % of the time block'], ['Routine', 'Routine'], ['en cours', 'in progress'],
  ['optionnel', 'optional'], ['cliquez une carte pour l\'avancer', 'click a card to move it forward'], ['non utilisé sur cette carte', 'not used on this card'],
  ['⚙ Personnaliser', '⚙ Customize'], ['+ Carte (Entrée)', '+ Card (Enter)'], ['Nouvelle carte du flux', 'New flow card'], ['Avancer d\'une colonne', 'Move forward one column'],
  ['Le Kata s\'ouvre une fois le run terminé', 'Kata opens once the run is finished'], ['Run terminé · cliquez le cadenas 🔒 pour passer au Kata', 'Run finished · click the lock 🔒 to move to Kata'],
  ['Run validé ✓', 'Run validated ✓'], ['Run impossible · obstacle ajouté au Parking Lot', 'Run blocked · obstacle added to the Parking Lot'],
  ['Storyboard', 'Storyboard'], ['Lecture seule. Le storyboard devient éditable une fois le run validé.', 'Read-only. The storyboard becomes editable once the run is validated.'],
  ['Éditable. Définissez la prochaine expérience.', 'Editable. Define the next experiment.'], ['Terminer le time block', 'Finish the time block'], ['Cycle enregistré ✓', 'Cycle saved ✓'],
  ['Historique de Kata', 'Kata history'], ['Clore la Target Condition', 'Close the Target Condition'], ['Période', 'Period'], ['· en cours', '· in progress'],
  ['Aucune Target Condition pour l\'instant.', 'No Target Condition yet.'], ['Target Condition sans résultat', 'Target Condition without result'],
  ['Valider la lecture', 'Confirm the reading'], ['Passer au Kata', 'Move to Kata'], ['Glisser pour réordonner', 'Drag to reorder'],
  ['Utiliser le flux sur cette carte', 'Use the flow on this card'], ['Colonnes : étapes du diagramme As Is', 'Columns: As Is diagram steps'],
  ['Challenge non défini : complétez-le dans le storyboard à l\'étape Kata.', 'Challenge not defined: fill it in the storyboard at the Kata step.'],
  ['Durée du time block (min)', 'Time block length (min)'], ['Ajoutez des étapes au diagramme As Is (Current Condition) pour créer les colonnes intermédiaires.', 'Add steps to the As Is diagram (Current Condition) to create the intermediate columns.'],
  ['Target Condition close · définissez la suivante dans le storyboard', 'Target Condition closed · define the next one in the storyboard'],
  ['Clore la Target Condition en cours ? Elle passe dans l\'historique et une nouvelle Target Condition vierge la remplace.', 'Close the current Target Condition? It moves to the history and a new blank Target Condition replaces it.'], ['Créer une section ici', 'Create a section here'], ['Déplacer vers la section', 'Move to section'], ['Sans section', 'No section'], ['Renommer la section', 'Rename section'], ['Replier la section', 'Collapse section'], ['Déplier la section', 'Expand section'], ['Supprimer la section', 'Delete section'], ['Glisser une propriété ici', 'Drag a property here'], ['Glisser pour déplacer', 'Drag to move'], ['Nom de la section', 'Section name'], ['Dans la page', 'In the page'], ['Toujours afficher', 'Always show'], ['Masquer si vide', 'Hide when empty'],
...
const I18N_PATTERNS = [
  ...
  [/^Supprimer les (\d+) Visions de démo \? Tes propres cartes ne sont pas touchées\. \(⌘Z annule\)$/, /^Delete the (\d+) demo Visions\? Your own cards are not affected\. \(⌘Z undoes\)$/, n => ({ fr: `Supprimer les ${n} Visions de démo ? Tes propres cartes ne sont pas touchées. (⌘Z annule)`, en: `Delete the ${n} demo Visions? Your own cards are not affected. (⌘Z undoes)` })],
  [/^(\d+) Visions de démo supprimées$/, /^(\d+) demo Visions deleted$/, n => ({ fr: `${n} Visions de démo supprimées`, en: `${n} demo Visions deleted` })],
  [/^Reste : routine (\d+)\/(\d+)(?: · flux (\d+)\/(\d+))?$/, /^Left: routine (\d+)\/(\d+)(?: · flow (\d+)\/(\d+))?$/, (a, b, c, d) => ({ fr: `Reste : routine ${a}/${b}${c !== undefined ? ` · flux ${c}/${d}` : ''}`, en: `Left: routine ${a}/${b}${c !== undefined ? ` · flow ${c}/${d}` : ''}` })],
  [/^Routine (\d+)\/(\d+)(?: · Flux (\d+)\/(\d+))?$/, /^Routine (\d+)\/(\d+)(?: · Flow (\d+)\/(\d+))?$/, (a, b, c, d) => ({ fr: `Routine ${a}/${b}${c !== undefined ? ` · Flux ${c}/${d}` : ''}`, en: `Routine ${a}/${b}${c !== undefined ? ` · Flow ${c}/${d}` : ''}` })],
  [/^(\d+) \/ (\d+) fait$/, /^(\d+) \/ (\d+) done$/, (a, b) => ({ fr: `${a} / ${b} fait`, en: `${a} / ${b} done` })],
  [/^(\d+) Target Conditions?$/, /^(\d+) Target Conditions?$/, n => ({ fr: `${n} Target Condition${n > 1 ? 's' : ''}`, en: `${n} Target Condition${n > 1 ? 's' : ''}` })],
  [/^Éditable\. Complétez l'expérience #(\d+) \(Happened \/ Learned\), puis définissez la suivante\.$/, /^Editable\. Complete experiment #(\d+) \(Happened \/ Learned\), then define the next one\.$/, n => ({ fr: `Éditable. Complétez l'expérience #${n} (Happened / Learned), puis définissez la suivante.`, en: `Editable. Complete experiment #${n} (Happened / Learned), then define the next one.` })],
  [/^cliquez le cercle pour enregistrer le cycle(?: · prochain run dans (\d+) j)?$/, /^click the circle to save the cycle(?: · next run in (\d+) d)?$/, n => ({ fr: `cliquez le cercle pour enregistrer le cycle${n ? ` · prochain run dans ${n} j` : ''}`, en: `click the circle to save the cycle${n ? ` · next run in ${n} d` : ''}` })],
